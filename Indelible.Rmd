...
indelible.db[,length.cut:=cut(abs(size),breaks=c(-1,0,1,2,3,4,5,6,10,20,50,100,500,10000,1000000000))]
...
print(paste0("Peak sensitivity (21-50bp)     : ",sprintf("%0.01f",sens[!is.na(sens),max(sens)]*100)))
...
additive.plot <- ggplot(additive.table,aes(set,count,group=cat,fill=cat)) + 
  geom_col(size=1,colour="black",alpha = 0.5) + 
  ...
  scale_alpha_continuous(range = c(0,1)) +
  scale_fill_manual(values = c(col.pal.plot[1,col],col.pal.plot[2,col],col.pal.plot[10,col],col.pal.plot[4,col]), guide = guide_legend(title = "Calling Algorithm")) +
...
size.plot <- ggplot(indelible.db[is.sv.resolved==T], aes(length.cut)) + 
  ...
  scale_x_discrete(name="Variant Length",labels=c("0","1","2","3","4","5","6","7-10","11-20","21-50","51-100","101-500","501-10,000","10,000+","Unknown")) + 
  scale_y_continuous(name = "Number of Clusters") +
  theme
size.plot
...
paste0("Peak sensitivity (20-50)     : ",sprintf(sp.format,sens[!is.na(sens),max(sens)]*100))
